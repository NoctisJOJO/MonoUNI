...
class Tester(object):
    def __init__(self, cfg, model, data_loader, logger):
        ...
        self.label_dir = os.path.join( self.root_dir, "label_2_4cls_filter_with_roi_for_eval")  ###  验证、测试集 真值
        # self.label_dir = os.path.join(self.root_dir, "label_2_4cls_for_train")
    ...
    def test(self):
        ...
        for batch_idx, (
            ...
            # # 立即保存当前批次的结果
            # for img_id, det in dets.items():
            #     out_path = os.path.join(out_dir, "data", f"{img_id}.txt")
            #     with open(out_path, "w") as f:
            #         for item in det:
            #             class_name = self.class_name[int(item[0])]
            #             line = f"{class_name} 0.0 0" + " ".join(
            #                 f"{x:.2f}" for x in item[1:]
            #             )
            #             f.write(line + "\n")

            # # 释放内存
            # del inputs, outputs, dets, calibs, denorms
